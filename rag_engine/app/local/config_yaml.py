...
commented defaults on first run. Only imported/used when PROFILE=local, so PyYAML
...
# Edit this file, then restart the daemon (or choose "Reload config" from the tray
# menu, if offered) to apply changes. Anything left commented out uses the built-in
# default. Secrets (like an OpenAI API key) are better set as an environment variable
# than pasted here, since this file is not encrypted.
...
def write_default_local_config(path: Path) -> None:
    path.parent.mkdir(parents=True, exist_ok=True)
    path.write_text(DEFAULT_CONFIG_TEMPLATE, encoding="utf-8")
    logger.info("Wrote default local config to %s", path)
...
def load_local_yaml_overrides(path: Path | None = None) -> dict:
    ...
    with commented defaults first if it doesn't exist yet."""
